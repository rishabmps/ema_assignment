...
export function TravelerSustainableBookingView() {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      addMessage({ id: generateUniqueId(), sender: "agent", content: "Got it. Searching for options to Boston...", type: "loading" });
    }, 500);
  ...
  const handleConfirmBooking = (cost: number) => {
    ...
    setShowBudget(false);
    setMessages(prev => prev.filter(m => m.type !== "card" && m.type !== "text"));
